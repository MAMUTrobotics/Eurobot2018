...
namespace Menu
{
  ...
  public:
    ...
    const   char * getLabel (const Item_t * item = NULL) const;
       var * getValue (const Item_t * item) ;
       var * getValue () ;
...
#define MenuItem(Name, Label, Next, Previous, Parent, Child, Callback, Value_index) \
  extern const Menu::Item_t Next, Previous, Parent, Child; \
  const Menu::Item_t PROGMEM Name = { &Next, &Previous, &Parent, &Child, &Callback, (const var*)Value_index, Label }
