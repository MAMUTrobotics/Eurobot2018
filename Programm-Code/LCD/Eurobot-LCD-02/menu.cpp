...
namespace Menu {
	...
	const Item_t NullItem PROGMEM = { (const Menu::Item_s *)NULL, (const Menu::Item_s *)NULL, (const Menu::Item_s *)NULL, (const Menu::Item_s *)NULL, (const Menu::Callback_t)NULL, (const var*)NULL, (const char *)NULL };
	...
	var * Engine::getValue(const Item_t * item) {
		return (var *)pgm_read_word((item == NULL) ? &currentItem->Value_index : &item->Value_index);
	}

	var * Engine::getValue() {
		return (var *)&currentItem->Value_index;
	}	
