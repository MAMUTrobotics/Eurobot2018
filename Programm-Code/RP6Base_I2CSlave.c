...
#include "RP6I2CslaveTWI.c"

/*****************************************************************************/

// The Slave Address on the I2C Bus can be specified here:
#define RP6BASE_I2C_SLAVE_ADR 12
...
void task_commandProcessor(void)
{
	...
	{writeString_P("\nKommando wurde aufgenommen\n");
		switch(cmd) 
		{
			...
			case CMD_SETLEDS: writeString_P("\nLEDs werden geschaltet\n");setLEDs(param1); break;
			//case CMD_STOP: stop(); break;
			case CMD_STOP: mSleep(50);writeString_P("\n Schlafe\n"); break;   // Zur vermeidung von Schreibzugriffen auf die Read-Register
			case CMD_MOVE_AT_SPEED:writeString_P("\nBewege mit Geschw. aktiv\n"); moveAtSpeed(param1, param2); break;
